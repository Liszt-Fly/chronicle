...
let showSubFolder = (event: MouseEvent, file: msfile) => {
    if (file.isDirectory) {
        ...
        console.log(fs.readFileSync(path.resolve(basePath.value, file.name!)) as unknown as string)
        content.value = fs.readFileSync(path.resolve(basePath.value, file.name!)) as unknown as string
