...
const enter = (event: KeyboardEvent) => {
  ...
  if (item.type == ChronicleNode.paragraph) {
    editor.value!.children[index].innerHTML = editor.value!.children[
      index
    ].textContent!.replaceAll(
      /\*(.+)\*/g,
      `<b style="background-color:#1abc9c;color:white" >$1</b>`
    );
    ...
    editor.value!.children[index].innerHTML = editor.value!.children[
      ...
      /\~~(.+)\~~/g,
      `<b style="text-decoration:line-through;" >$1</b>`
    );
  }
...
const backspace = (event: KeyboardEvent) => {
  ...
  if (editor.value!.children[index].textContent == "") {
    ...
    setTimeout(() => {
      ...
      editor.value!.children[index - 1].textContent =
        Parser.currentNodeParser.content;
      //* 光标回到上一行的末尾
...
<template>
  <div class="column">
    ...
    <div class="column-right">
      <!-- 右侧 editor -->
      <div class="editor" :contenteditable="bContentedible" ref="editor" @keydown.enter="enter($event)"
        @keydown.backspace="backspace($event)" @keydown.ctrl="save" @keydown.down="down" @keydown.up="up">
